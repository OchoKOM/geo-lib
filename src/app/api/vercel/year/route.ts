...
export async function GET(req: Request) {
    ...
    console.log('--- DÉCLENCHEMENT DU CRON JOB (1er Janvier) : VÉRIFICATION DE LA PROCHAINE ANNÉE ACADÉMIQUE ---');
  ...
  try {
    ...
    if (existingYear) {
      console.log(`[SUCCÈS] L'année académique ${yearString} existe déjà. Aucune action nécessaire.`);
    ...
    console.log(`[CRÉÉ] Nouvelle année académique insérée : ${yearString} (Du ${startDate.toLocaleDateString()} au ${endDate.toLocaleDateString()})`);
    ...
    console.error(`[ERREUR] Impossible de traiter l'année ${yearString}:`, error);
    throw new Error(`Erreur lors de l'ajout de l'année académique: ${yearString}`);
  } finally {
