...
export default function Dashboard () {
  ...
  // Seuls ADMIN et LIBRARIAN peuvent supprimer. AUTHOR peut publier et modifier.
  const isAuthorized = ['ADMIN', 'LIBRARIAN', 'AUTHOR'].includes(role)
  const canDelete = ['ADMIN', 'LIBRARIAN'].includes(role)

  // Hook UploadThing configuré sur la route 'documentUploader'
  ...
  // Modifié pour inclure le fichier à uploader
  const [newBook, setNewBook] = useState({
    title: '',
    description: '',
    type: 'TFC' as BookType,
    studyAreaId: '',
    fileToUpload: null as File | null // Nouveau champ
  })
  ...
  const fetchStats = async () => {
    try {
      const data = await kyInstance
        .get('/api/dashboard/stats', { throwHttpErrors: false })
        .json<{
          stats: {
            totalBooks: number
            totalAuthors: number
  ...
  const fetchBooks = async () => {
    ...
    try {
      ...
      const data = await kyInstance.get(url).json<{ data: any[] }>()
      if (data.data) {
        setBooks(data.data)
        if (searchBounds) {
          showToast(
            `${data.data.length} document(s) trouvé(s) dans la zone.`,
            'success'
          )
  ...
  useEffect(() => {
    ...
    if (miniMapRef.current) {
      ...
      miniMapTileLayerRef.current = L.tileLayer(
  ...
  const searchStudyAreas = async (query: string) => {
    ...
    const data = await kyInstance
      .get(`/api/study-areas?q=${query}`)
      .json<{ results: any }>()
    if (data.results) setFoundAreas(data.results)
  }

  const handlePublish = async () => {
    // Validation : Seul le titre est obligatoire maintenant
    if (!newBook.title) {
      showToast('Le titre du document est requis', 'warning')
      return
    }
    ...
    try {
      ...
      if (newBook.fileToUpload) {
        const uploadResult = await startUpload([newBook.fileToUpload])
      ...
      if (data) {
        ...
        showToast('Erreur de publication', 'destructive')
      }
    } catch (e: any) {
      showToast(`Erreur: ${e.message || 'Erreur serveur'}`, 'destructive')
    } finally {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (editingBook.fileToUpload) {
        ...
        uploadedFileName = fileData.name
        uploadedFileSize = fileData.size
        showToast('Nouveau fichier PDF téléversé avec succès', 'success')
      }
  ...
  const openDeleteDialog = (book: Book) => {
    ...
    setIsEditDialogOpen(false) // Ferme la modale d'édition
  }

  const handleDelete = async () => {
    if (!bookToDelete?.id) return

    setIsUpdating(true) // On réutilise isUpdating pour l'état de chargement
    try {
      const data = await kyInstance
        .delete(`/api/books?id=${bookToDelete.id}`, {
          headers: { 'Content-Type': 'application/json' }
        })
        .json()

      if (data) {
        showToast(`Document "${bookToDelete.title}" supprimé.`, 'success')
        setIsDeleteDialogOpen(false)
        setBookToDelete(null)
        fetchBooks() // Rafraîchit la liste
        fetchStats() // Rafraîchit les statistiques
      } else {
        showToast(
          data.error || 'Erreur lors de la suppression du livre.',
          'destructive'
        )
      }
    } catch (e) {
      showToast(
        'Erreur serveur lors de la tentative de suppression.',
        'destructive'
      )
      console.error(e)
    } finally {
    ...
    <div className='min-h-screen bg-slate-50 dark:bg-slate-950 p-6 font-sans text-slate-900 dark:text-slate-100'>
          ...
          <TabsContent value='library' className='space-y-4'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
              {books.map(book => (
                <div
                  key={book.id}
                  className='bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-md transition-shadow flex flex-col'
                >
                  ...
                  <div className='bg-slate-50 dark:bg-slate-950 p-3 border-t border-slate-200 dark:border-slate-800'>
                    ...
                    <div className='flex flex-wrap gap-1'>
                      {book.studyAreas.length > 0 ? (
                        book.studyAreas.map(link => (
                          <span
                            key={link.studyArea.id}
                            className='text-[10px] px-2 py-0.5 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded-full border border-blue-200 dark:border-blue-800'
                          >
                            {link.studyArea.name}
                          </span>
                        ))
          ...
          <TabsContent value='admin'>
            <div className='bg-white dark:bg-slate-900 p-8 rounded-lg border border-slate-200 dark:border-slate-800 text-center'>
              ...
              <p className='text-slate-500 max-w-md mx-auto'>
                Utilisez le bouton &quot;Publier un Document&quot; pour ajouter
                du contenu ou l&apos;icône crayon sur les cartes pour modifier.
              </p>
            </div>
          </TabsContent>
        </Tabs>
            ...
            <div className='p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900'>
              <div>
                <h3 className='font-bold flex items-center gap-2'>
                  <MapIcon className='w-5 h-5 text-blue-600' /> Filtre
                  Géographique
                </h3>
                <p className='text-xs text-slate-500'>
                  Dessinez un rectangle pour voir les livres liés aux zones dans
                  cette région.
                </p>
              </div>
              <Button
                ...
                {searchBounds ? (
                  <div className='text-green-600 font-medium flex flex-col gap-2'>
                    <span className='flex items-center gap-2'>
                      <Check className='w-4 h-4' /> Zone active
                    </span>
                    <Button
                      size='sm'
      ...
      <Dialog open={isPublishDialogOpen} onOpenChange={setIsPublishDialogOpen}>
        <DialogContent className='sm:max-w-[600px]'>
          <DialogHeader>
            <DialogTitle>Publier un document</DialogTitle>
            <DialogDescription>
          ...
          <div className='grid gap-4 py-4'>
            <div className='grid gap-2'>
              <Label className='text-sm font-medium'>Titre (Obligatoire)</Label>
              <Input
                value={newBook.title}
                onChange={e =>
            ...
            <div className='grid grid-cols-2 gap-4'>
              <div className='grid gap-2'>
                ...
                <Select
                  value={newBook.type}
                  onValueChange={(val: BookType) =>
                    setNewBook({ ...newBook, type: val })
                  }
                >
                  <SelectTrigger>
            ...
            <div className='grid gap-2'>
              ...
              {newBook.fileToUpload && (
                <p className='text-xs text-green-600 font-medium'>
                  Fichier sélectionné : {newBook.fileToUpload.name}
                </p>
              )}
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                isPublishing || isUploadThingUploading || !newBook.title
              }
            >
              {isPublishing || isUploadThingUploading ? (
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className='sm:max-w-[700px]'>
          <DialogHeader>
            ...
            <DialogDescription>
              Mettez à jour les informations, liez de nouvelles zones
              géographiques et ajoutez des fichiers.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className='grid gap-6 py-4 overflow-y-auto max-h-[70vh] pr-2'>
              ...
              <div className='grid gap-2'>
                ...
                <Select
                  ...
                  onValueChange={(val: any) =>
                    setEditingBook({ ...editingBook, type: val })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              <div className='space-y-3 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-800'>
                ...
                <div className='grid gap-2'>
                  {role === 'ADMIN' ? (
                    <Select
                      value={editingBook.author?.id ?? ''}
                      onValueChange={val =>
                        setEditingBook({
                          ...editingBook,
                          author: authors.find(a => a.id === val)
                        })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue placeholder='Changer auteur' />
                      </SelectTrigger>
                      <SelectContent>
                        {authors.map(author => (
                    ...
                    <>
                      ...
                      <p className='text-[10px] text-slate-400'>
                        Pour changer l&apos;auteur, veuillez contacter un
                        administrateur.
                      </p>
                    </>
                  )}
              ...
              <div className='space-y-3 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-800'>
                ...
                <div className='flex flex-wrap gap-2 mb-2'>
                  {editingBook.studyAreas?.map((sa, idx) => (
                    <div
                      key={idx}
                      className='bg-white dark:bg-slate-800 px-2 py-1 rounded border border-slate-200 dark:border-slate-700 text-xs flex items-center gap-1 shadow-sm'
                    >
                      {sa.studyArea.name}
                    </div>
                  ))}
                  {(!editingBook.studyAreas ||
                    editingBook.studyAreas.length === 0) && (
                  ...
                  <div className='flex gap-2'>
                    <Input
                      ...
                      onChange={e =>
                        setEditingBook({
                          ...editingBook,
                          newStudyAreaId: e.target.value
                        })
                      }
                    />
                  </div>
                  <p className='text-[10px] text-slate-500'>
                    Collez l&apos;identifiant unique (UUID) de la zone
                    géographique ici.
                  </p>
                </div>
              </div>
                  ...
                  <div className='flex items-center gap-2 overflow-hidden'>
                    ...
                    <div className='min-w-0'>
                      <p className='text-sm font-medium truncate'>
                        {editingBook.documentFile.name}
                      </p>
                      <a
                        href={editingBook.documentFile.url}
                        target='_blank'
            ...
            {canDelete && (
              <div className='space-y-3 p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-900'>
                <h4 className='text-sm font-semibold text-red-600 dark:text-red-400 uppercase tracking-wider mb-2 flex items-center gap-2'>
                  <Trash className='w-4 h-4' /> Zone de Danger
                </h4>
                <p className='text-sm text-red-700 dark:text-red-300'>
                  Cette action est irréversible. La suppression retire
                  définitivement le document et son entrée de fichier de la base
                  de données.
                </p>
                <Button
                  variant='destructive'
                  onClick={() => openDeleteDialog(editingBook as Book)}
                >
                  <Trash className='w-4 h-4 mr-2' /> Supprimer Définitivement le
                  Livre
                </Button>
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isUpdating || isUploadThingUploading}
              className='bg-blue-600 hover:bg-blue-700'
            >
              {isUpdating || isUploadThingUploading ? (
                <Loader2 className='w-4 h-4 animate-spin mr-2' />
              ) : (
      ...
      {/* --- ALERTE DE CONFIRMATION DE SUPPRESSION (NOUVEAU) --- */}
      <AlertDialog
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className='text-red-600 flex items-center gap-2'>
              <Trash className='w-5 h-5' /> Confirmation de Suppression
            </AlertDialogTitle>
            <AlertDialogDescription>
              Êtes-vous absolument certain de vouloir supprimer le document :{' '}
              <span className='font-bold'>
                &quot;{bookToDelete?.title}&quot;
              </span>{' '}
              ? Cette action est **irréversible** et le document sera retiré de
              la bibliothèque.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
