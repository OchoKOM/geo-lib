...
import {
  updateStudyArea,
  createStudyAreaFromLayer,
  getAvailableStudyAreas
} from '../actions'
import {
  MapIcon,
  PenTool,
...
export default function EditMapClient ({
  ...
  const [attributeTableLayerId, setAttributeTableLayerId] = useState<
    string | 'all'
  >(
    initialGeoJson && initialGeoJson.features.length > 0
      ? 'existing-poly'
      : 'all'
  )
  ...
  const [editingStyleLayerId, setEditingStyleLayerId] = useState<string | null>(
    null
  )
  ...
  useEffect(() => {
    if (initialGeoJson && initialGeoJson.features) {
      ...
      const addInitLayer = (
        ...
        if (
          typesFound.has(key as GeoJSON.Geometry['type']) ||
          typesFound.has(`Multi${key}` as GeoJSON.Geometry['type'])
        ) {
          const layer = {
            id,
            name,
            ...
            visible: true,
            showLabels: false,
            count: initializedFeatures.filter(f =>
              f.geometry.type.includes(key)
            ).length,
          ...
          initializedFeatures
            .filter(f => f.geometry.type.includes(key))
            .forEach(f => {
    ...
    getAvailableStudyAreas().then(
      (areas: SetStateAction<ImportableStudyArea[]>) => {
    ...
    mapInstance.on(L.Draw.Event.CREATED, (e: L.LeafletEvent) => {
      ...
      if (!targetLayerConfig || targetLayerConfig.id === 'new') {
        const geoType = (
          geoJson.geometry.type.includes('Polygon')
            ? 'Polygon'
            : geoJson.geometry.type.includes('LineString')
            ? 'LineString'
            : 'Point'
        ) as GeometryMode
        targetLayerConfig =
          activeLayers.find(l => l.type === geoType && !l.isDatabaseBound) ||
          activeLayers.find(l => l.type === geoType)
      }
      ...
      if (targetLayerConfig) {
        ...
        setActiveLayers(prev =>
          prev.map(l =>
            l.id === targetLayerConfig?.id ? { ...l, count: l.count + 1 } : l
          )
        )
      }
  ...
  useEffect(() => {
    ...
    features.forEach(feature => {
      ...
      const layerId =
        feature.properties._layerId ||
        (geoType.includes('Polygon')
          ? activeLayers.find(l => l.type === 'Polygon')?.id
          : geoType.includes('LineString')
          ? activeLayers.find(l => l.type === 'LineString')?.id
          : geoType.includes('Point')
          ? activeLayers.find(l => l.type === 'Point')?.id
          : null)
      ...
      layer.eachLayer(l => {
        ...
        const tooltipContent = `
            <div class="font-semibold text-sm">${
              feature.properties[layerConfig.labelProperty || 'id'] ||
              'Sans nom'
            }</div>
        ...
        l.bindTooltip(tooltipContent, {
          ...
          className: layerConfig?.showLabels
            ? 'bg-card border-none shadow-none text-foreground font-bold text-shadow'
            : 'bg-none shadow px-2 py-1 rounded border',
        ...
        if (feature.id === selectedFeatureId) {
          if (l instanceof L.Path) {
            l.setStyle({
              color: '#fbbf24',
              weight: (layerConfig?.style.weight || 2) + 3,
              dashArray: '5, 5'
            })
            if (l instanceof L.CircleMarker)
              l.setStyle({ fillColor: '#fbbf24', radius: 10 })
          }
        }
  ...
  const handleZoomToLayer = (layerId: string) => {
    ...
    const layerFeatures = features.filter(
      f => f.properties._layerId === layerId
    )
  ...
  const handleToggleLayerLabels = (layerId: string) => {
    setActiveLayers(prev =>
      prev.map(l =>
        l.id === layerId ? { ...l, showLabels: !l.showLabels } : l
      )
    )
  }
  ...
  const handleConfirmSaveLayer = async (name: string, description: string) => {
    ...
    const layerFeatures = features.filter(
      f => f.properties._layerId === layerToSaveId
    )
    ...
    const file = new File([blob], `${name.replace(/\s+/g, '_')}.geojson`, {
      type: 'application/geo+json'
    })
    ...
    try {
      ...
      const getMultiType = (
        ...
        switch (type) {
          case 'Point':
            return 'MultiPoint' as const
          case 'LineString':
            return 'MultiLineString' as const
          case 'Polygon':
            return 'MultiPolygon' as const
          default:
            return 'MultiPolygon' as const
        }
      }
      ...
      if (result.success) {
        ...
        setActiveLayers(prev =>
          prev.map(l =>
            l.id === layerToSaveId
              ? { ...l, isDatabaseBound: true, name: name }
              : l
          )
        )
      } else {
  ...
  const handleImportLayer = async (area: ImportableStudyArea) => {
    try {
      ...
      const geoJson = res
      // @ts-expect-error - An
      const newFeatures = (geoJson.features || [geoJson])
        .map((f: GeoJSON.Feature, i: number) => ({
          ...f,
          id: `imp-${area.id}-${i}`,
  ...
  const handleStartEditing = () => {
    if (targetLayer === 'new') {
      ...
      let targetLayerConfig = activeLayers.find(
        l => l.type === newLayerType && !l.isDatabaseBound
      )
    ...
    showToast(
      `Mode édition activé : ${
        editMode === 'None' ? newLayerType : 'Couche existante'
      }`,
      'default'
    )
  }
  ...
  const addFeatureProperty = () => {
    ...
    if (key)
      setFeatures(prev =>
        prev.map(f => ({ ...f, properties: { ...f.properties, [key]: '' } }))
      )
  }
  const updateLayerStyle = (layerId: string, newStyle: Partial<LayerStyle>) => {
    setActiveLayers(prev =>
      prev.map(l =>
        l.id === layerId ? { ...l, style: { ...l.style, ...newStyle } } : l
      )
    )
  }
  const deleteFeature = (id: string | number) => {
    if (confirm('Supprimer ?')) {
      ...
      if (featureToDelete && featureToDelete.properties._layerId) {
        setActiveLayers(prev =>
          prev.map(l =>
            l.id === featureToDelete.properties._layerId
              ? { ...l, count: l.count - 1 }
              : l
          )
        )
  ...
  const featuresToList = useMemo(() => {
    if (attributeTableLayerId === 'all')
      return features.filter(f => f.properties._layerId)
      ...
      <header className='flex-none border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 flex items-center justify-between shadow-sm z-30 max-md:flex-wrap gap-3'>
        ...
        <div className='flex items-center gap-2 w-full justify-end flex-wrap'>
          <Button
            variant='outline'
            size='sm'
            onClick={() => setIsAttributeTableOpen(true)}
            className='flex gap-2 max-sm:rounded-full max-sm:size-10'
          >
            <span className='max-sm:sr-only'>Table Attributaire</span>
            <Maximize2 className='w-4 h-4 max-sm:hidden' />
          ...
          {editMode === 'None' ? (
            ...
            >
              <span className='max-sm:sr-only'>Éditer Couches</span>
            ...
            <Button
              ...
              className='flex gap-2  max-sm:rounded-full max-sm:size-10'
            >
              <span className='max-sm:sr-only'>Terminer</span>
