...
import { 
  Save, Layers, Book, TableProperties, AlertCircle, 
  Plus, Trash2, Map as MapIcon, Loader2, 
  PenTool, Settings2, X, Eye, EyeOff, MousePointer2,
  CheckCircle2
} from 'lucide-react'
...
export default function EditMapClient({ studyArea, initialGeoJson, initialBooks }: EditMapClientProps) {
  ...
  useEffect(() => {
    ...
    if (initialGeoJson) {
      const layerCounts: Record<string, number> = { Polygon: 0, LineString: 0, Point: 0 }
      ...
      const layer = L.geoJSON(initialGeoJson, {
        onEachFeature: (feature, l) => {
          ...
          if (type === 'Polygon' || type === 'MultiPolygon') layerCounts.Polygon++
          if (type === 'LineString' || type === 'MultiLineString') layerCounts.LineString++
      ...
      if (layerCounts.Polygon > 0) detectedLayers.push({ id: 'existing-poly', name: 'Polygones (Existants)', type: 'Polygon', visible: true, count: layerCounts.Polygon })
      if (layerCounts.LineString > 0) detectedLayers.push({ id: 'existing-line', name: 'Lignes (Existantes)', type: 'LineString', visible: true, count: layerCounts.LineString })
      if (layerCounts.Point > 0) detectedLayers.push({ id: 'existing-point', name: 'Points (Existants)', type: 'Point', visible: true, count: layerCounts.Point })
    ...
    mapInstance.on(L.Draw.Event.CREATED, (e) => {
      ...
      const type = e.layerType === 'marker' ? 'Point' : (e.layerType === 'polyline' ? 'LineString' : 'Polygon')
      updateLayerCount(type, 1)
    })

    mapInstance.on(L.Draw.Event.DELETED, (e) => {
       // Simplification: on ne recalcule pas tout ici pour l'exemple
    })
  ...
  useEffect(() => {
    ...
    const drawOptions = {
      edit: {
        ...
        edit: true    // Toujours permettre la modif de sommets
      },
      draw: {
        polygon: false,
        rectangle: false,
  ...
  const updateLayerCount = (type: string, delta: number) => {
    setActiveLayers(prev => {
      const exists = prev.find(l => l.type === type)
      if (exists) {
        return prev.map(l => l.type === type ? { ...l, count: l.count + delta } : l)
      } else {
        // Création d'une nouvelle entrée dans la légende si elle n'existe pas
        const name = type === 'Polygon' ? 'Nouveaux Polygones' : (type === 'LineString' ? 'Nouvelles Lignes' : 'Nouveaux Points')
        return [...prev, { id: `new-${type.toLowerCase()}`, name, type: type as GeometryMode, visible: true, count: 1 }]
      }
    })
  }
  ...
  const handleStartEditing = () => {
    ...
    showToast(`Mode édition activé : ${targetLayer === 'new' ? newLayerType : 'Couche existante'}`, 'default')
  }
  ...
  const handlePropChange = (key: string, value: string) => setProperties(prev => ({ ...prev, [key]: value }))
  const addProperty = () => {
  ...
  const removeProperty = (key: string) => {
    if (confirm(`Supprimer "${key}" ?`)) {
      const newProps = { ...properties }; delete newProps[key]; setProperties(newProps)
    }
  }
      ...
      <Dialog open={isLayerDialogOpen} onOpenChange={setIsLayerDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Sélectionnez la couche à modifier ou définissez un nouveau type de géométrie à ajouter.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              <Label>Action</Label>
              <RadioGroup value={targetLayer} onValueChange={setTargetLayer} className="flex flex-col space-y-1">
                <div className="flex items-center space-x-2 border p-3 rounded-md cursor-pointer hover:bg-muted-foreground/30">
                  ...
                  <Label htmlFor="r-new" className="cursor-pointer font-medium">Créer une nouvelle couche</Label>
                </div>
                {activeLayers.map(layer => (
                  <div key={layer.id} className="flex items-center space-x-2 border p-3 rounded-md cursor-pointer hover:bg-muted-foreground/30">
                    <RadioGroupItem value={layer.id} id={`r-${layer.id}`} />
                    <Label htmlFor={`r-${layer.id}`} className="cursor-pointer flex items-center gap-2">
                      {layer.type === 'Polygon' && <Layers className="w-3 h-3 text-blue-500"/>}
                      {layer.type === 'LineString' && <Layers className="w-3 h-3 text-green-500"/>}
                      {layer.type === 'Point' && <Layers className="w-3 h-3 text-red-500"/>}
                      Modifier {layer.name} <span className="text-xs text-slate-400">({layer.count} objets)</span>
                    </Label>
                  </div>
                ))}
              </RadioGroup>
            ...
            {targetLayer === 'new' && (
              <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                ...
                <Select value={newLayerType} onValueChange={(v: GeometryMode) => setNewLayerType(v)}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="Polygon">Polygone / Multipolygone (Zones)</SelectItem>
                    <SelectItem value="LineString">Ligne / Multiligne (Routes, Rivières)</SelectItem>
                    <SelectItem value="Point">Point / Multipoint (Sites)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsLayerDialogOpen(false)}>Annuler</Button>
        ...
        <div className="flex items-center gap-3">
          ...
          {editMode !== 'None' && (
             <div className="flex items-center gap-2 px-3 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-full text-xs font-semibold animate-pulse">
                ...
                Édition : {editMode === 'Polygon' ? 'Polygones' : (editMode === 'LineString' ? 'Lignes' : 'Points')}
             </div>
          )}
        </div>
      ...
      <div className="flex-1 flex relative overflow-hidden">
        ...
        >
          <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800">
            <h2 className="font-semibold text-sm uppercase tracking-wide text-slate-500">Inspecteur SIG</h2>
          ...
          <form action={(formData) => { prepareFormData(formData); formAction(formData); }} className="flex-1 flex flex-col overflow-hidden">
            <input type="hidden" name="id" value={studyArea.id} />
            
            <Tabs defaultValue="layers" className="flex-1 flex flex-col h-full">
              <div className="px-4 pt-4">
                <TabsList className="w-full grid grid-cols-3">
                  <TabsTrigger value="layers"><Layers className="w-4 h-4 mr-2"/>Couches</TabsTrigger>
                  <TabsTrigger value="attributes"><TableProperties className="w-4 h-4 mr-2"/>Données</TabsTrigger>
                  <TabsTrigger value="info"><Book className="w-4 h-4 mr-2"/>Infos</TabsTrigger>
                </TabsList>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 h-full">
                ...
                <TabsContent value="layers" className="space-y-4 m-0 h-full">
                  <div className="space-y-3">
                    <p className="text-xs text-slate-500 font-medium uppercase mb-2">Structure de la carte</p>
                    
                    {activeLayers.length === 0 && (
                      <div className="text-center p-4 border border-dashed rounded text-sm text-slate-500">
                        Aucune géométrie. Cliquez sur &quot;Mode Édition&quot; pour commencer.
                      </div>
                    )}

                    {activeLayers.map((layer) => (
                      <div key={layer.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] text-slate-500">{layer.count} objets • {layer.type}</span>
                          </div>
                        </div>
                        {editMode === layer.type && (
                          <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" title="Couche active" />
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  {editMode !== 'None' && (
                    <div className="mt-6 p-3 bg-blue-50 border border-blue-100 rounded text-xs text-blue-800">
                      <p className="font-semibold mb-1">Aide à l&apos;édition :</p>
                      <ul className="list-disc pl-4 space-y-1">
                        <li>Utilisez la barre d&apos;outils sur la carte pour dessiner.</li>
                        <li>Vous ne pouvez dessiner que des <b>{editMode === 'Polygon' ? 'Polygones' : editMode}</b> actuellement.</li>
                        <li>Cliquez sur &quot;Terminer l&apos;édition&quot; pour changer de couche.</li>
                      </ul>
                    </div>
                  )}
                </TabsContent>
                ...
                <TabsContent value="attributes" className="space-y-4 m-0">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-sm font-semibold">Table Attributaire</h3>
                  ...
                  <div className="space-y-3">
                    {Object.entries(properties).map(([key, value]) => (
                      <div key={key} className="space-y-1 p-2 bg-slate-50 dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-700">
                        ...
                        <Input value={value} onChange={(e) => handlePropChange(key, e.target.value)} className="h-8 text-sm" />
                      </div>
                    ))}
                  </div>
                </TabsContent>
                ...
                <TabsContent value="info" className="space-y-4 m-0">
                  ...
                  <div className="mt-4">
                     <h3 className="text-sm font-semibold mb-2">Livres associés</h3>
                     {initialBooks.map((book) => (
                        <div key={book.id} className="text-xs p-2 border mb-1 rounded">
                           {book.title}
                        </div>
                     ))}
                  </div>
