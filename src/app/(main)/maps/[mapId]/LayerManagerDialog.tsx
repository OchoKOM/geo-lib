...
import { Palette, PenTool, Focus, CloudUpload, Database, DownloadCloud } from 'lucide-react'
import { GeometryMode, LayerConfig, LayerStyle, ImportableStudyArea } from './map-types'
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-[900px] h-[80vh] flex flex-col p-0 overflow-hidden'>
        ...
        <div className='flex-1 grid grid-cols-1 md:grid-cols-12 gap-0 overflow-hidden'>
          ...
          <div className='md:col-span-4 border-r border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col'>
             <div className="p-4 border-b">
                 <Label className='text-xs font-semibold uppercase text-muted-foreground mb-2 block'>Couches Actives</Label>
                 <RadioGroup
                    value={targetLayer}
                    onValueChange={(val) => {
                        setTargetLayer(val)
                 ...
                 >
                    {activeLayers.map(layer => (
                        ...
                        >
                            <div className='flex items-center space-x-2 overflow-hidden'>
                                ...
                                <div className='flex flex-col truncate'>
                                    ...
                                    <span className='text-[10px] text-muted-foreground flex items-center gap-1'>
                                        ...
                                        {layer.isDatabaseBound && <Database className="w-3 h-3 text-blue-500"/>}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                <Button variant="ghost" size="icon" className="h-6 w-6" title="Zoomer sur la couche" onClick={(e) => { e.stopPropagation(); onZoomToLayer(layer.id); }}>
                                    <Focus className="w-3 h-3" />
                                </Button>
                                {!layer.isDatabaseBound && (
                                    <Button variant="ghost" size="icon" className="h-6 w-6 text-blue-600" title="Sauvegarder en BDD" onClick={(e) => { e.stopPropagation(); onSaveLayerDb(layer.id); }}>
                    ...
                    <div 
                        className={`flex items-center space-x-2 border p-2 rounded-md transition-colors cursor-pointer text-sm mt-4
                            ${targetLayer === 'new' ? 'bg-blue-50 border-blue-200 text-blue-700' : 'hover:bg-slate-100'}
                        `}
                        onClick={() => setTargetLayer('new')}
                    >
                        <RadioGroupItem value='new' id='r-new' />
                        <Label htmlFor='r-new' className='cursor-pointer flex-1 font-medium'>+ Créer une couche</Label>
                    </div>
                 </RadioGroup>
             </div>
             ...
             <div className="flex-1 overflow-hidden flex flex-col">
                 ...
                 <ScrollArea className="flex-1 p-2">
                     {isLoadingDBLayers ? (
                         <div className="text-xs text-center p-4 text-muted-foreground">Chargement...</div>
                     ) : availableDBLayers.length === 0 ? (
                         <div className="text-xs text-center p-4 text-muted-foreground">Aucune zone disponible.</div>
                     ) : (
                         <div className="space-y-2">
                             {availableDBLayers.map(area => (
                                 <div key={area.id} className="flex items-center justify-between p-2 border rounded bg-white dark:bg-slate-900 text-xs">
                                     <div className="truncate pr-2">
                                         <div className="font-medium truncate">{area.name}</div>
                                         <div className="text-muted-foreground">{area.type}</div>
                                     </div>
                                     <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => onImportLayer(area)}>
             ...
             {targetLayer === 'new' ? (
                <div className="flex flex-col items-center justify-center h-full space-y-6">
                    <div className='w-full max-w-sm p-6 border rounded-xl bg-slate-50 dark:bg-slate-900'>
                        <Label className='text-sm font-semibold mb-4 block'>Configuration de la nouvelle couche</Label>
                        
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <Label className="text-xs text-muted-foreground">Type de géométrie</Label>
                                <Select value={newLayerType} onValueChange={(v: GeometryMode) => setNewLayerType(v)}>
                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value='Polygon'>Polygone (Zones)</SelectItem>
                                        <SelectItem value='LineString'>Ligne (Routes/Rivières)</SelectItem>
                                        <SelectItem value='Point'>Point (Lieux d&apos;intérêt)</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            
                            <Button onClick={onStartEditing} className='w-full bg-blue-600 hover:bg-blue-700'>
                        ...
                        return (
                            <div className='space-y-6 p-6 bg-slate-50 dark:bg-slate-900 rounded-xl border'>
                                <div className='grid grid-cols-2 gap-8'>
                                    <div>
                                        <Label className='text-xs font-semibold uppercase text-muted-foreground mb-3 block'>Remplissage</Label>
                                        <div className="space-y-4">
                                            <div className='flex items-center gap-3'>
                                                ...
                                                />
                                                <span className='text-xs font-mono'>{layer.style.fillColor}</span>
                                            </div>
                                            <div className='space-y-2'>
                                    ...
                                    <div>
                                        <Label className='text-xs font-semibold uppercase text-muted-foreground mb-3 block'>Contour</Label>
                                        <div className="space-y-4">
                                            <div className='flex items-center gap-3'>
                                                <input 
                                                    type="color" 
                                                    ...
                                                    className="w-10 h-10 rounded cursor-pointer border-0 p-0"
                                                />
                                                <span className='text-xs font-mono'>{layer.style.color}</span>
                                            </div>
                                            <div className='space-y-2'>
