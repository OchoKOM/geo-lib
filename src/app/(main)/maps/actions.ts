...
function geojsonToWKT(geometry: any): string {
  ...
  switch (type) {
    ...
    case 'MultiPoint':
      return `MULTIPOINT(${coords.map((c: any) => formatCoord(c)).join(', ')})`

    case 'LineString':
      return `LINESTRING(${coords.map((c: any) => formatCoord(c)).join(', ')})`

    case 'MultiLineString':
      return `MULTILINESTRING(${coords.map((line: any[]) => 
        `(${line.map(c => formatCoord(c)).join(', ')})`
      ).join(', ')})`

    case 'Polygon':
      return `POLYGON(${coords.map((ring: any[]) => 
        `(${ring.map(c => formatCoord(c)).join(', ')})`
      ).join(', ')})`

    case 'MultiPolygon':
      return `MULTIPOLYGON(${coords.map((poly: any[]) => 
        `(${poly.map((ring: any[]) => 
          `(${ring.map(c => formatCoord(c)).join(', ')})`
        ).join(', ')})`
      ).join(', ')})`
...
export async function updateStudyArea(
  ...
  if (!user) {
    return { success: false, message: "Vous devez être connecté pour modifier une zone." }
  }

  // Vérification du rôle (Admin, Bibliothécaire ou Auteur)
  const dbUser = await prisma.user.findUnique({ where: { id: user.id } })
  if (!dbUser || !['ADMIN', 'LIBRARIAN', 'AUTHOR'].includes(dbUser.role)) {
    return { success: false, message: "Vous n'avez pas les droits pour modifier cette carte." }
  }
  ...
  try {
    ...
    if (geojsonData.type === 'FeatureCollection') {
      ...
      if (count > 1 && (firstType === 'Polygon' || firstType === 'MultiPolygon')) {
        geometry = {
          type: 'MultiPolygon',
          coordinates: geojsonData.features.map((f: any) => 
    ...
    return { success: false, message: `Erreur lors de la sauvegarde: ${(error as Error).message}` }
  }
}