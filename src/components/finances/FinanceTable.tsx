...
export function FinanceTable({
  ...
  const triggerDeleteConfirm = (type: 'loans' | 'subscriptions', id: string) => {
        ...
        <tbody className='divide-y divide-slate-100 dark:divide-slate-800'>
          {activeTab === 'loans'
                ...
                <tr key={loan.id} className='group hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors'>
                    ...
                    <div className='flex items-center gap-3'>
                      ...
                      <div>
                        <p className='font-medium text-slate-900 dark:text-slate-100'>
                          {loan.user?.name || loan.user?.username || 'Utilisateur inconnu'}
                        </p>
                  ...
                  <td className='px-6 py-4 text-right'>
                    <div className='flex justify-end items-center gap-2'>
                      ...
                      >
                        {printingId === loan.id ? <Loader2 className='w-4 h-4 animate-spin' /> : <Printer className='w-4 h-4' />}
                      </Button>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align='end' className='w-56'>
                          ...
                          {!loan.returnDate && onMarkReturned && (
                            <DropdownMenuItem onClick={() => onMarkReturned(loan)} className='text-emerald-600'>
                              <CheckCircle className='w-4 h-4 mr-2' /> Marquer comme retourné
                            </DropdownMenuItem>
                          )}
                          ...
                          {isAuthorized(UserRole.ADMIN) && (
                            <DropdownMenuItem 
                              ...
                              onClick={() => triggerDeleteConfirm('loans', loan.id)}
                            >
                              <Trash2 className='w-4 h-4 mr-2' /> Supprimer le prêt
                            </DropdownMenuItem>
                          )}
                        </DropdownMenuContent>
                      </DropdownMenu>
            ...
            : (data as DashboardSubscription[]).map(sub => (
                <tr key={sub.id} className='group hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors'>
                  <td className='px-6 py-4'>
                    <div className='flex items-center gap-3'>
                      <div className='w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500'>
                        <User className='w-4 h-4' />
                      ...
                      <div>
                        <p className='font-medium text-slate-900 dark:text-slate-100'>{sub.user?.username || 'Utilisateur inconnu'}</p>
                        <p className='text-xs text-slate-500'>{sub.user?.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className='px-6 py-4 text-sm text-slate-600 dark:text-slate-400'>
                    Du {new Date(sub.startDate).toLocaleDateString()} au {new Date(sub.endDate).toLocaleDateString()}
                  </td>
                  <td className='px-6 py-4'>
                    {(() => {
                      const daysLeft = Math.ceil((new Date(sub.endDate).getTime() - new Date().getTime()) / (1000 * 3600 * 24))
                      if (!sub.isActive) return <span className='text-slate-400'>-</span>
                      return (
                  ...
                  <td className='px-6 py-4'>
                    {sub.isActive ? (
                      <Badge className='bg-emerald-100 text-emerald-800 border-emerald-200 dark:bg-emerald-900 dark:text-emerald-300'>Actif</Badge>
                    ) : (
                  ...
                  <td className='px-6 py-4 text-right'>
                    <div className='flex justify-end gap-2 items-center'>
                      <Button size='icon' variant='ghost' className='h-8 w-8' onClick={() => onEdit(sub)}>
                      ...
                      {isAuthorized(UserRole.ADMIN) && (
                        <Button 
                          ...
                          onClick={() => triggerDeleteConfirm('subscriptions', sub.id)}
                        >
