...
const Textarea = (props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => (
  <textarea
    {...props}
    className={`flex min-h-20 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-800 dark:bg-slate-950 dark:ring-offset-slate-950 dark:placeholder:text-slate-400 dark:focus-visible:ring-slate-300 ${props.className}`}
  />
)
          ...
          <>
            <AlertDialogHeader>
              <AlertDialogTitle className='flex items-center gap-2 text-blue-600 dark:text-blue-400'>
                <Database className='w-5 h-5' /> Sauvegarder la Zone
                d&apos;Étude
              </AlertDialogTitle>
              <AlertDialogDescription>
                Définissez les propriétés de cette couche pour
            ...
            <div className='grid gap-4 py-4'>
              ...
              <div className='grid gap-2'>
                <label htmlFor='layer-desc' className='text-sm font-medium'>
          ...
          <>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className='text-center'>
                La zone a été ajoutée à la base de données. Voici son
                identifiant unique.
              </AlertDialogDescription>
            </AlertDialogHeader>
            ...
            <div className='py-6'>
              ...
              <p className='text-xs text-slate-400 mt-2'>
                Copiez cet ID pour l&apos;utiliser dans vos configurations ou
                API.
              </p>
            </div>
    ...
    <div className='p-4 border-t border-slate-100 dark:border-slate-800 space-y-3 bg-slate-50 dark:bg-slate-900/50'>
      ...
      <div>
        ...
        <Input
          ...
          value={layer.opacity}
          onChange={e =>
            onUpdateStyle(layer.id, layer.color, parseFloat(e.target.value))
          }
      ...
      <div className='flex flex-col gap-2 pt-2 border-t border-slate-200 dark:border-slate-700'>
        <Button
          ...
          disabled={
            !isAuthenticated || !['ADMIN', 'LIBRARIAN', 'AUTHOR'].includes(role)
          }
        >
...
const LayerList = ({
  ...
  return (
    <div
      className={`flex flex-1 flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 transition-all duration-300 z-20 shadow-xl ${
        sidebarOpen
          ? 'w-80 translate-x-0'
    ...
    >
      ...
      <div className='p-2 space-y-2 overflow-y-auto flex-1'>
        {!layers.length && (
          <div className='flex h-full flex-col items-center justify-center text-muted-foreground'>
            ...
            <p className='p-4 text-center'>
              Aucune couche chargée. Importez des fichiers ou recherchez dans la
              base de données.
            </p>
          </div>
        )}
        {layers.map((layer: LayerData) => (
          ...
          >
            <div
              className='flex flex-col items-center shrink-0 w-8 h-8 rounded-full border border-slate-300 dark:border-slate-700'
...
const AttributeTable = ({
  ...
  return (
    <div
      className={`relative h-full bg-white dark:bg-slate-900 shadow-2xl border-t border-slate-200 dark:border-slate-800 duration-300 ease-in-out z-30 flex w-fit max-w-full`}
    >
      <div
        className='flex flex-col items-center justify-center p-4 cursor-pointer bg-slate-100 dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 min-h-screen w-12'
        onClick={() => setExpanded(!expanded)}
      >
        <h2 className='font-bold flex items-center gap-4 text-sm uppercase tracking-wider text-slate-600 dark:text-slate-400 [writing-mode:vertical-lr] rotate-180'>
          <span>Table des Attributs {layerName ? `(${layerName})` : ''}</span>
          <TableIcon className='w-4 h-4 rotate-180' />
        </h2>

        <Button size='sm' variant='ghost' className='mt-4 h-6 w-6 p-0'>
          {expanded ? (
      ...
      <div
        className={`transition-all duration-200 ${
          expanded ? 'opacity-100 visible w-full' : 'opacity-0 invisible w-0'
        }`}
      >
        <div className='flex-1 overflow-auto h-full'>
          {data.length > 0 ? (
            <div className='w-full overflow-auto'>
              <table className='min-w-full divide-y divide-slate-200 dark:divide-slate-700 overflow-auto'>
...
export default function GeoMap () {
  ...
  useEffect(() => {
    ...
    tileLayerRef.current = L.tileLayer(lightUrl, {
      maxZoom: 19,
    ...
    try {
      ...
      for (const base in shapefileGroup) {
        ...
        if (group.shp && group.dbf) {
          ...
          const results = await shp(content).then(r =>
            Array.isArray(r) ? r : [r]
          )
          results.forEach(fc =>
            candidates.push({
              id: crypto.randomUUID(),
              name: fc.fileName || base,
              originalName: base,
              data: fc,
              type: 'shapefile',
      ...
      for (const file of zipFiles) {
        const results = await shp(await file.arrayBuffer()).then(r =>
          Array.isArray(r) ? r : [r]
        )
        results.forEach((fc, idx) =>
          candidates.push({
            id: crypto.randomUUID(),
            name: fc.fileName || `Couche ${idx + 1}`,
            originalName: file.name,
            data: fc,
            type: 'shapefile',
            featureCount: fc.features?.length || 0,
      ...
      if (candidates.length === 0) {
        ...
        showToast('Aucune donnée valide.', 'warning')
      }
    } catch (e) {
      console.log(e)
  ...
  const loadSearchResult = async (result: SearchResult) => {
    ...
    try {
      const data = result.geojsonFile?.url
        ? await kyInstance.get(result.geojsonFile.url).json<{
      ...
      addLayerToMap({
        ...
        opacity: 1,
        data,
        featureCount: Array.isArray(data.features) ? data.features.length : 1,
        generatedId: false,
        generatedGeom: false
      })
      setIsSearchDialogOpen(false)
      ...
      setLoadingResults(p => ({ ...p, [result.id]: false }))
    }
  }
  ...
  const addLayerToMap = (layerData: LayerData) => {
          ...
          L.popup({ maxWidth: 300 })
            ...
            .setContent(
              `<div class="font-bold text-sm mb-1">${
                layerData.name
              }</div><div class="text-xs text-slate-700">${Object.entries(
                feature.properties || {}
              )
                .slice(0, 3)
    ...
    try {
      ...
      if (b.isValid()) mapRef.current.fitBounds(b, { padding: [50, 50] })
    } catch (e) {
      console.log(e)
    }
  ...
  const deleteLayer = (id: string) => {
    if (layerInstancesRef.current[id] && mapRef.current)
      mapRef.current.removeLayer(layerInstancesRef.current[id])
  ...
  const loadAttributes = (layer: LayerData) => {
    const feats =
      (layer.data as { features: { [key: string]: unknown }[] }).features || []
    if (!feats.length) {
      setTableData([])
      setTableColumns([])
      return
    ...
    setTableData(
      feats.map((f: { [key: string]: unknown }, i: number) => ({
        _fid: i,
        ...(f.properties || {})
      }))
    )
    ...
    if (format === 'geojson') {
      ...
      try {
        const zip = await shpwrite.zip(
          {
            type: 'FeatureCollection',
            features: (layer.data as { features: [] }).features
          },
          {
            folder: layer.name,
            outputType: 'blob',
        ...
        showToast('Erreur export SHP.', 'destructive')
      }
    }
  }
  ...
  if (!isMounted)
    return (
      <div className='h-[calc(100vh-64px)] w-full flex items-center justify-center bg-slate-50 dark:bg-slate-950'>
        <Loader2 className='w-10 h-10 animate-spin text-blue-600' />
      </div>
    )

  return (
    <div className='flex top-0 h-[calc(100vh-64px)] w-full overflow-hidden relative bg-slate-100 dark:bg-slate-950 font-sans text-slate-800 dark:text-slate-200'>
      {/* MAP & ATTRIBUTES */}
      <div className='flex-1'>
        <div ref={mapContainerRef} className='absolute inset-0 z-10' />

        <div className='absolute top-0 right-0 flex h-full transition-all duration-300 z-20 max-md:w-full justify-end'>
          {!sidebarOpen && (
            <Button
              size='sm'
              className={cn('relative flex gap-2 top-4 max-md:bottom-16 right-4 z-20 shadow-lg', tableExpanded && 'max-md:hidden')}
              onClick={() => setSidebarOpen(true)}
            >
              <Layers className='w-4 h-4' /> Voir les Couches
            </Button>
          )}
      ...
      <div
        className={`flex flex-col z-20 ${
          tableExpanded ? 'max-md:w-0 transition-none' : 'max-md:w-auto'
        }`}
      >
        <LayerList
          layers={layers}
          ...
          onSelectLayer={(l: LayerData) => {
            setSelectedLayerId(l.id)
            loadAttributes(l)
          }}
          onToggleVisibility={(id: string) => {
            const l = layerInstancesRef.current[id]
            if (l) {
              if (mapRef.current?.hasLayer(l)) mapRef.current.removeLayer(l)
              else mapRef.current?.addLayer(l)
            }
            setLayers(p =>
      ...
      <ImportModal
        ...
        onToggleCandidate={id =>
          setImportCandidates(p =>
            p.map(c => (c.id === id ? { ...c, selected: !c.selected } : c))
          )
        }
        onToggleAll={sel =>
          setImportCandidates(p => p.map(c => ({ ...c, selected: sel })))
        }
