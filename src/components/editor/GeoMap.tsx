...
const AttributeTable = ({ data, columns, expanded, setExpanded, layerName }: any) => {
 return (
 ...
 {col === '_fid' ? row._fid : row[col] !== undefined ? String(row[col]) : 'N/A'}
 </td>
 ))}
 </tr>
 ))}
...
export default function GeoMap() {
 ...
 } catch (e) { showToast("Impossible de charger la géométrie.", 'destructive') } 
 ...
 l.eachLayer((x: any) => x.setStyle && x.setStyle({ fillColor: color, fillOpacity: opacity * 0.8, opacity }))
 ...
 const cols = new Set<string>(); feats.forEach((f: any) => f.properties && Object.keys(f.properties).forEach(k => cols.add(k)))
 setTableColumns(Array.from(cols))
 setTableData(feats.map((f: any, i: number) => ({ _fid: i, ...f.properties })))
 }
 ...
 try {
 const zip = await shpwrite.zip({ type: 'FeatureCollection', features: layer.data.features }, {
 ...
 onFileImport={(e: any) => { if(e.target.files) processFiles(e.target.files); e.target.value = '' }}
 onFolderImport={(e: any) => { if(e.target.files) processFiles(e.target.files); e.target.value = '' }}
 />
